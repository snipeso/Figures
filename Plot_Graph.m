% plots a graph of all the pairs to compare
% CSV format: Column 1 and 2 contains nodes. Column 3 contains value of edge
% to run quickly with: Plot_Nodes('C:\Users\Sophia\Projects\Matlab_Mini_Scripts\OPTCLA.csv', 80, false)

function Plot_Graph(filepath, cutoff, create_fig)
% filepath: full path to csv
% cutoff: lowest value to display, on scale of 1-100
% create_fig: true/false, if this is creating the figure or not
...
CSV.Properties.VariableNames = {'Node_1' 'Node_2' 'Value'}; % change names because csv usually has junk strings

% select pairs that have value above cutoff
Edges = CSV(CSV.Value > cutoff, :);
Edges = sortrows(Edges, 'Value'); % sort in order so that darker lines will be on top

% identify unique nodes
Nodes = unique([CSV.Node_1; CSV.Node_2]);
...
% create figure if this does not exist
if not(create_fig)
    figure(1)
end
